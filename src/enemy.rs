...
use crate::{ActiveEnemies, WinSize, Materials, SCALE, Enemy, Laser, FromEnemy, Speed, TIME_STEP, MAX_ENEMIES, MAX_FORMATION_MEMBERS};
...
impl FormationMaker {
    fn make(&mut self, win_size: &WinSize) -> Formation {
        match (
            ...
            // If still within the formation cound
            (Some(tmpl), false) => {
                self.current_formation_members += 1;
                tmpl.clone()
            }
...
fn enemy_movement(
    ...
    for (mut tf, speed, mut formation) in query.iter_mut() {
        ...
        // Start rotating the formation angle only when sprite are on or close to desination
